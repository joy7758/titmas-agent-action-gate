...
| Gate-input time-of-check bypass | untrusted PR tests execute pre-existing dirty tracked bytes or rewrite task, policy, evidence, approval, Git state, Action configuration or reserved outputs before authorization | pre-test exact-HEAD tracked index/worktree admission; bounded declared untracked inputs; canonical in-memory snapshots; post-test path, ancestor, directory, inode, empty-content, digest, effective-config, exact-head and Git-state recheck; verified create-only outputs; mutation fails closed | task/evidence/policy/approval/multi-input/dirty-HEAD/Git-state/in-place-output/directory-replacement/action/symlink tests |
| PR test credential or execution-capability inheritance | untrusted tests inherit GitHub command files, OIDC, SSH, cloud, package-registry or effective repository/worktree Git command and credential configuration | minimal environment allowlist, fresh HOME/TMPDIR, complete effective repository/worktree Git configuration preflight without value retention, `pull_request_target` rejection and process-group cleanup | fake credential, local/worktree/include/URL/SSH/filter config, event, aggregate-output and cleanup tests |
