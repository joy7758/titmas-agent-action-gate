...
class TestPolicy(unittest.TestCase):
    def test_default_policy_path_from_env(self) -> None:
        with patch.dict(os.environ, {"TITMAS_ACTION_GATE_POLICY_PATH": "/tmp/test-policy.json"}), patch("pathlib.Path.is_file", return_value=True):
            self.assertEqual(default_policy_path(), Path("/tmp/test-policy.json"))

    def test_default_policy_path_not_found(self) -> None:
        with (
            patch.dict(os.environ, {"TITMAS_ACTION_GATE_POLICY_PATH": "/tmp/missing-policy.json"}, clear=True),
            patch("pathlib.Path.is_file", return_value=False),
            self.assertRaisesRegex(ContractValidationError, "Pinned GitHub policy file was not found."),
        ):
            default_policy_path()
    ...
    def test_policy_engine_evaluate_valid_request(self) -> None:
        policy = {
            ...
            "version": "1.0.0",
            "supersedes": "none",
            "default": {"effect": "DENY", "risk_class": "CRITICAL", "required_evidence_types": ["SOURCE_PIN", "REQUEST_AUTHORIZATION"]},
            "rules": [
                {
        ...
        request = {
            ...
            "action": "github.branch.push",
            "target": {"provider": "github", "repository": "org/repo", "resource_ref": "main"},
